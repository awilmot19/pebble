...
static void draw_tennis(Layer *layer, GContext *ctx) {
  ...
  // paddles
  graphics_draw_line(ctx, GPoint(cpu_x_pos, USER_GOAL), GPoint(cpu_x_pos + PADDLE_WIDTH, USER_GOAL));
  graphics_draw_line(ctx, GPoint(person_x_pos, CPU_GOAL), GPoint(person_x_pos + PADDLE_WIDTH, CPU_GOAL));
...
static void move() {
  if (ball_x_pos>(cpu_x_pos+PADDLE_HEIGHT/2)+5) {
    cpu_vel = CPU_VEL;
  } else if (ball_x_pos<(cpu_x_pos-PADDLE_HEIGHT/2)-5) {
    cpu_vel = -CPU_VEL;
  } else {
    cpu_vel = 0;
  }
  ...
  if (ball_x_pos>PADDLE_DISTANCE-BALL_RADIUS && ball_x_pos<=PADDLE_DISTANCE+BALL_RADIUS) {
    if (ball_y_pos>=person_x_pos && ball_y_pos<=person_x_pos+PADDLE_HEIGHT) {
      if (ball_x_vel < 0) {
        ball_x_vel = -ball_x_vel;
        ball_y_vel = (ball_y_pos - person_x_pos - PADDLE_HEIGHT/2)*VERT_VEL*2;
        ball_y_vel = ball_y_vel/PADDLE_HEIGHT;
      }
    }
  }
  if (ball_x_pos>=WIDTH-PADDLE_DISTANCE-BALL_RADIUS && ball_x_pos<WIDTH-PADDLE_DISTANCE+BALL_RADIUS) {
    if (ball_y_pos>=cpu_x_pos && ball_y_pos<=cpu_x_pos+PADDLE_HEIGHT) {
      if (ball_x_vel > 0) {
        ball_x_vel = -ball_x_vel;
        ball_y_vel = (ball_y_pos - cpu_x_pos - PADDLE_HEIGHT/2)*VERT_VEL*2;
        ball_y_vel = ball_y_vel/PADDLE_HEIGHT;
      }
    }
  }
